...
arr = _.uniq(arr); // не должно быть повторяющихся скиллов
arr = _.sortBy(arr); // отсортированы по алфавиту
console.log(arr);

// Массив имен (поле name) людей отсортированных в зависимости от количества их друзей
var names = [];
var i = 0;
_.map(_.sortBy(data,"friends"), function(value) {
	names[i] = value.name;
	i++;
});
...
value.friends.forEach(function(value) {
	friends = _.concat(friends,value.name);
});
});
